...
import DeviceItemMobile from "../SettingPageItemMobile/DeviceItemMobile";
import DefaultPlusIcon from "../../../../components/icons/PlusIcon/DefaultPlusIcon";

const ContentDeviceScreenMobile = ({listItems, addDeviceAction, activeDetailDeviceAction}) => {
    return (
        <>
            ...
            <div className="grid grid-cols-2 gap-[2rem] mt-[2rem]">
                {
                    listItems.map((item, index) =>
                        <DeviceItemMobile icon={item.icon} name={item.name} room={item.room} key={item + index} unit={item.unit}
                                          value={item.value}
...
export default ContentDeviceScreenMobile;